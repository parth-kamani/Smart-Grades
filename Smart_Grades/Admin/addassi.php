...
<?php
	...
	if(isset($_POST['submitb']))
	{
		$file = $_FILES['image']['tmp_name'];
			...
			move_uploaded_file($_FILES["image"]["tmp_name"],"assignment/" . $_FILES["image"]["name"]);
			
			$location="assignment/" . $_FILES["image"]["name"];
			touch("assignment/" .$image_name);
			...
			$ext = end($re);
			rename("assignment/" .$image_name,"assignment/".$rnd.".".$ext);
			$location="assignment/".$rnd.".".$ext;
		if(isset($_GET['sel']))
		{
			$ch_num = $_POST['ch_n'];
			$name = $_POST['name'];
			...
			$cdate = $_POST['cdate'];
			$am = $_POST['a_m'];
			$rowsql = mysqli_query($con,"SELECT MAX(a_id) AS max FROM tbl_stuassi WHERE cor_id=".$_GET['sel']);
			
		$row = mysqli_fetch_array($rowsql);
		$cid = $row['max'];
		$cid++;
		$rowsql1 = mysqli_query($con,"SELECT MAX(a_num) AS max FROM tbl_stuassi WHERE cor_id=".$_GET['sel']." AND ch_id=".$ch_num);
			
		$row1 = mysqli_fetch_array($rowsql1);
		$cid1 = $row1['max'];
		$cid1++;
		$m=$_GET['sel'];
		$query = "INSERT INTO tbl_stuassi (a_num,ch_id,a_name,a_desc,a_dline,a_mrk,a_url,cor_id,valid) VALUES('$cid1','$ch_num','$name','$desc','$cdate','$am','$location','$m','1')";
		...
		$result = mysqli_query($con,$query);
			...
			else
			{
				...
				header("refresh:0; url=addassi.php?flag=1");
...
<head>
  ...
  <title>Smart Grades | Add Assignment</title>
      ...
      <!-- Small boxes (Stat box) -->
	   ...
	   <div class="box box-warning">
			...
			<?php
				...
				if($_GET['flag']==1)
					{
						echo "<center><font style='color:green; text-align:center'>Assignment Added Successfully</font></center><br/>";
...
<br/>
				<div class="form-group">
				<label><small>*NOTE: Please verify the assignment before filling up details.</small></label>
                  <center><label>Select Assignment To Upload</label></center>
				<center><button id="reupload-button" class="btn btn-primary" onclick="reupload()">Re-Upload</button></center>  
<button id="upload-button" class="btn btn-primary">Select Assignment PDF</button> 
	...
	if(__CURRENT_PAGE != __TOTAL_PAGES)
		showPage(++__CURRENT_PAGE);
				...
				<!--<div class="form-group">
                  ...
                  <input type="number" name="num" class="form-control" placeholder="Enter Assignment Number" value="<?php echo $c; ?>" readonly />
                </div>-->
                <div class="form-group">
                  <label>Assignment Name</label>
                  <input type="text" name="name" class="form-control" placeholder="Enter Assignment Name" required />
                </div>
                <!-- textarea -->
                <div class="form-group">
                  <label>Assignment Description</label>
                  <textarea class="form-control" rows="3" name="desc" placeholder="Enter Assignment Description" required></textarea>
                </div>
				<div class="form-group">
                  <label>Assignment Marks</label>
                  <input type="number" name="a_m" class="form-control" placeholder="Enter Assignment Marks" required />
                </div>
				<div class="form-group" >
                <label>Deadline Date:</label>
					...
					<input type="text" name="cdate" class="form-control pull-right" id="datepicker" placeholder="Enter Assignment Deadline Date" autocomplete="off" required />
					</div>
					</div>
