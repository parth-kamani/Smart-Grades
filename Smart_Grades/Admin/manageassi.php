...
<?php
 ...
 $qw=$o;
	...
	if(isset($_GET['qt']))
	{
		//$gp=$_GET['qr'];
		$fp=$_GET['qt'];
		$t="manageassi.php?sel=".$_GET['sel']."&&qt=".$_GET['qt']."&&show=all";
			   $y="manageassi.php?sel=".$_GET['sel']."&&qt=".$_GET['qt']."&&show=active";
			   $u="manageassi.php?sel=".$_GET['sel']."&&qt=".$_GET['qt']."&&show=non-active";
...
<?php
	...
	if(isset($_GET['del']))
	{
		...
		$sql1="UPDATE tbl_stuassi SET valid=0 WHERE a_id=".$_GET['del']."";
		$result=mysqli_query($con,$sql1);
		header("location:manageassi.php");
	}
?>
...
<head>
  ...
  <title>Smart Grades | Manage Assignment</title>
...
<div id="h2" style="display: none;">
	...
	<a class="btn btn-primary" id ="active" href=<?php echo $y ?>>Active</a>
    <a class="btn btn-primary" id ="n-active" href=<?php echo $u ?>>Non-active</a>
              ...
              <table class="table table-hover" id="tab">
				...
				<?php
					...
					if(isset($_GET['show']))
					{
						...
						if($show == "all")
						{
							...
							";
							$query="SELECT * FROM tbl_stuassi WHERE cor_id=$m AND ch_id=$fp ORDER BY a_num ASC";
						}
						else if($show == "active")
						{	
							echo "
						...
						else if($show == 'non-active')
						{
							...
							";
							$query="SELECT * FROM tbl_stuassi WHERE cor_id=$m AND ch_id=$fp And valid=0 ORDER BY a_num ASC";
						}
					}
                ...
                <tr>
				  ...
				  <button type="button" onclick="p2()" class="btn btn-info btn-xs view_data" data-toggle="modal" id="<?php echo $value2['a_id'];?>">VIEW</button>&nbsp;&nbsp;
					...
					<a class="btn btn-primary btn-xs" href="editassi.php?id=<?php echo $value2['a_id'];?> " 
 ...
 $(document).ready(function(){
           ...
           {  
                $.ajax({  
                     url:"viewassi.php?sel="+<?php echo $_GET['sel'];?>+"&&qt="+<?php echo $_GET['qt'];?>+"",  
                     method:"POST",  
                     data:{ch_id:ch_id},  
                     success:function(data){  
                          $('#ch_detail').html(data);  
...
$(document).ready(function(){
 ...
 {
  $.ajax({
   url:"fetchassi2.php?sel="+<?php echo $_GET['sel'];?>+"&&qt="+<?php echo $_GET['qt'];?>,
   method:"POST",
   data:{query:query},
   success:function(data)
   {
