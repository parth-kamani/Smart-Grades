...
<?php
	...
	if(isset($_GET['del']))
	{
		...
		$sql1="UPDATE tbl_chapters SET active=0 WHERE ch_id=".$_GET['del']."";
		$result=mysqli_query($con,$sql1);
		header("location:managechap.php");
	}
?>
...
<head>
  ...
  <title>Smart Grades | Manage Chapters</title>
...
else if($type==3)
{
	...
	if(isset($_GET['sel']))
	{
		...
		$m = $_GET['sel'];
			   ...
			   $t="managechap.php?sel=".$_GET['sel']."&&show=all";
			   $y="managechap.php?sel=".$_GET['sel']."&&show=active";
			   $u="managechap.php?sel=".$_GET['sel']."&&show=non-active";
 ...
 $('#class_search').keyup(function(){
  ...
  else
  {
   load_data();
    ...
    <a class="btn btn-primary" id ="non-active" href=<?php echo $u ?>>Non Active</a>
  </div>         
  </div>
            ...
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover" id="tab">
				...
				<?php
					...
					if(isset($_GET['show']))
					{
						...
						if($show == "all")
						{
							...
							";
							$query="SELECT * FROM tbl_chapters WHERE cor_id=$m ORDER BY ch_num ASC";
						}
						else if($show == "active")
						{	
							echo "
						...
						else if($show == 'non-active')
						{
							...
							";
							$query="SELECT * FROM tbl_chapters WHERE cor_id=$m AND active=0 ORDER BY ch_num ASC";
						}
					}
                ...
                <tr>
				  ...
				  <a class="btn btn-success btn-xs" href="editchap.php?id=<?php echo $value2['ch_id'];?> " 
					onclick="return confirm('sure to edit?');">EDIT</a> &nbsp;&nbsp;
				  <a class="btn btn-danger btn-xs" href="?del=<?php echo $value2['ch_id'];?>" 
...
$(document).ready(function(){
 ...
 {
  $.ajax({
   url:"fetchch2.php?sel="+<?php echo $_GET['sel'];?>,
   method:"POST",
   data:{query:query},
   success:function(data)
   {
