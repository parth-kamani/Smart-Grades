...
<?php
	...
	if(isset($_GET['del']))
	{
		...
		$sql1="UPDATE tbl_course SET active=0 WHERE cor_id=".$_GET['del']."";
...
<head>
  ...
  <title>Smart Grades | Manage Courses</title>
      ...
      <div class="row">
	   <div class="box box-warning">
			...
			<!--<div class="col-xs-2">-->
                  ...
                  <input type="text" name="class_search" id="class_search" class="form-control" autocomplete="off" placeholder="Search" value="<?php echo $mq; ?>" onkeydown="a()" />
			
	
            </div>
				<div style="float:right">
			  <div class="btn-group">
    <a class="btn btn-primary" id ="all" href="managecourses.php?show=all">All</a>
	<a class="btn btn-primary" id ="active" href="managecourses.php?show=active">Active</a>
    <a class="btn btn-primary" id ="non-active" href="managecourses.php?show=non-active">Non Active</a>
  </div>         
  </div>
				...
				<?php
					...
					if(isset($_GET['show']))
					{
						...
						if($show == "all")
						{
							...
							";
							$query="SELECT * FROM tbl_course WHERE cl_id='$c_id'";
						}
						else if($show == "active")
						{	
							echo "
							...
							";
							$query="SELECT * FROM tbl_course WHERE cl_id='$c_id' AND active=1";
						}
						else if($show == 'non-active')
						{
							...
							";
							$query="SELECT * FROM tbl_course WHERE cl_id='$c_id' AND active=0";
						}
					}
					...
					if($count>0)
						{
					while(($value2 = mysqli_fetch_array($result2)) && ($value3 = mysqli_fetch_array($result3)))
					{
						$hj=strlen($value2['cor_desc']);
	 if($hj>25)
	 {
	 $bv=substr($value2['cor_desc'],0,25);
	 $bv.="...";
	 }
	 else
	 {
	 $bv=$value2['cor_desc'];
                ...
                <tr>
                  ...
                  <td><?php echo $value2['cor_name'];?></td>
				  <td><?php echo $value2['cor_fee'];?></td>
                  <td><?php echo $bv;?></td>
				  <td><?php echo $value2['cor_days'];?></td>
                  <td><?php echo $value3['cl_name'];?></td>
					
				  <td> 
				  <button type="button" class="btn btn-info btn-xs view_data" data-toggle="modal" id="<?php echo $value2['cor_id'];?>">VIEW</button>&nbsp;&nbsp;
				  <?php
				  if($type==1)
				  {
					  ?>
				  <a class="btn btn-success btn-xs" href="editcourses.php?id=<?php echo $value2['cor_id'];?> " 
					onclick="return confirm('sure to edit?');">EDIT</a> &nbsp;&nbsp;
				  <a class="btn btn-danger btn-xs" href="?del=<?php echo $value2['cor_id'];?>" 
...
$(document).ready(function(){
 ...
 {
  $.ajax({
   url:"fetchcor2.php?sel=<?php echo $c_id; ?>",
   method:"POST",
   data:{query:query},
   success:function(data)
   {
